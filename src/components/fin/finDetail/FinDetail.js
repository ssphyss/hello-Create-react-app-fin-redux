...
import { actionCreators as actionCreatorsFin } from './../store';
...
class FinDetail extends React.Component{
    render(){    
        ...
        console.log('-----(FinDetail讀取)dataSearch', this.props.dataSearch)
        console.log('-----(FinDetail讀取)dataActive', this.props.dataActive)
        console.log('-----(FinDetail讀取)isShowModal', this.props.isShowModal)
        ...
        return(
            <div style={{ margin: '50px 20px', padding: '30px', backgroundColor: '#ffffff'}} >
                ...
                <Modal
                    ...
                    onOk= {()=>this.props.handleSubmit(this.props.dataActive)}
...
const mapDispathToProps = (dispatch) => {
    return {
        ...
        // 確定OK--改從別的組件
        handleSubmit(dataActive){
            console.log('確定') ;
            console.log('dataActive', dataActive) ;
            const action = actionCreatorsFin.getModalOK(dataActive);            
            dispatch(action);


            const action2 = actionCreators.getModalOK();
            dispatch(action2);
        },
